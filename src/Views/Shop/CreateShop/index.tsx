...
const CreateShop = ({navigation}: IAppScreenProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    phoneNumber: '',
    address: '',
    networks: [] as Array<ISocialNetwork>,
  });
  ...
  const saveShop = async () => {
    ...
    await createShop(
      {...formData, userId: String(authState.profile?.id)},
      networks ? networks : [],
    )
      .then(x => {
  ...
  const handleModalShowChange = (file?: ICameraFile[]) => {
    if (file) {
      setFile({
        ...
        filename: file[0]?.filename ? file[0]?.filename : '',
      });
      setShowCamera(!showCamera);
      setFormData({
        ...formData,
        profileImage: file[0]?.uri ? file[0]?.uri : '',
      });
    }
  };
                  ...
                  <View style={styles.content}>
                    ...
                    <View style={[styles.accordionHolder, {marginTop: 30}]}>
                      <TouchableOpacity
                        ...
                        onPress={() => accordion()}
                        style={styles.Button}>
                        <AppText fontSize={15} font="bolder">
                          Redes Sociales
                        </AppText>
                      </TouchableOpacity>
                      ...
                      <View style={{height: updatedHeight, overflow: 'hidden'}}>
                        <View style={CommonStyles.mt_2}>
                          <Input
                            ...
                            icon="facebook"
                            onChange={value =>
                              handleInputChange(value, 'facebook')
                            }
                          />
                        </View>
                        <View style={CommonStyles.mt_2}>
                          <Input
                            ...
                            onChange={value =>
                              handleInputChange(value, 'instagram')
                            }
                          />
                        </View>
                      </View>
