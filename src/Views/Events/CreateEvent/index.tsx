...
import { DatePickerAndroid, Image, ScrollView, StyleSheet, View } from "react-native";
...
const CreateEvent = ({ route, navigation }: IScreenProps) => {
    ...
    const [formData, setFormData] = useState({ title: '', description: '', dateStart: '', dateEnd: '' })
    const [datePickerStart, setDatePickerStart] = useState(false);
    const [datePickerEnd, setDatePickerEnd] = useState(false);
    const [formDate, setformDate] = useState({ dateStart: new Date(), dateEnd: new Date() })
    ...
    function onDateSelected(value: any, type: string) {
        ...
        console.log( value)
        const dateStart = new Date(value).getDate().toString() + '/' + String(new Date(value).getMonth() + 1) + '/' + new Date(value).getFullYear().toString()
        const dateEnd = new Date(value).getDate().toString() + '/' + String(new Date(value).getMonth() + 1) + '/' + new Date(value).getFullYear().toString()
        type === 'start' ? setFormData({ ...formData, dateStart: dateStart }) : setFormData({ ...formData, dateEnd: dateEnd })
        type === 'start' ? setformDate({ ...formDate, dateStart: value }) : setformDate({ ...formDate, dateEnd: value })
    };

    const validateForm = () => {
        return !formData.title || !formData.dateStart || !formData.description || !formData.dateEnd;
    }

    const saveEvent = async () => {
        await createEvent({ ...formData, dateEnd: formDate.dateEnd, shopId: route.params.id }).then(x => {
    ...
    return (
        <>
            <LinearGradient colors={['#1D5771', '#2A8187', '#46D9B5']} style={styles.container}>
                <AppText font='bolder' style={{ textAlign: 'center', marginTop: 40 }} fontSize={24}>Crear Evento</AppText>
                        ...
                        <View style={{ width: '80%', marginTop: 30 }}>
                            ...
                            {datePickerStart &&
                                <DateTimePicker
                                    ...
                                    mode={'date'}
                                    is24Hour={true}
                                    onChange={(value) => onDateSelected(value.nativeEvent.timestamp, 'start')}
                            ...
                            {datePickerEnd &&
                                <DateTimePicker
                                    ...
                                    mode={'date'}
                                    is24Hour={true}
                                    onChange={(value) => onDateSelected(value.nativeEvent.timestamp, 'end')}
                                    style={styles.datePicker}
                                />
                            }
                        </View>
